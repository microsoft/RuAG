...
class Alice_and_Bob(object):
    def __init__(self, map_size=(13, 9), n=2):
        ...
        self.agent_num = n
        self.generate_map()
        self.map = {'agent':{0: 5, 1: 6}, 'key':{0: 2, 1: 3}, 'lever': 4, 'treasure': 7, 'wall': 1}
    ...
    def generate_map(self, init_state=None):
        ...
        if init_state is None:
            ...
            if random_position:
                while True:
                    pos1 = [np.random.randint(1, 6), np.random.randint(1, 4)]
                    pos2 = [np.random.randint(1, 4), np.random.randint(self.width - 4, self.width - 1)]
                    # pos2 = [3, 5]
                    
                    self.agt_pos = [pos1, pos2]
                    if pos2 != [3, 5]:
                        break
            else:
                self.agt_pos = [[1, 1], [1, self.width-2]]
        else:
            relative_pos1 = init_state[0][11:13]
            relative_pos2 = init_state[1][11:13]
            pos1 = [11 + relative_pos1[0], 7 + relative_pos1[1]]
            pos2 = [11 + relative_pos2[0], 7 + relative_pos2[1]]
            self.agt_pos = [pos1, pos2]
    ...
    def get_info(self, ):
        return {
            'obs2block': self.map, # color2block
            'key_pos': self.keys_pos, # [[3, 5], [11, 1]]
            'door_pos': self.doors_pos, # [[8, 2], [4, 6]]
            'lever_pos': self.lever_pos, # [5, 7]
            'treasure_pos': self.treasure_pos # [11, 7]
        }
    def reset(self, init_state=None):
        self.generate_map(init_state)
        obs = self.get_obs()
        self.log = []
        return obs
    ...
    def step(self, action_list):
        ...
        # print(next_obs)
        # if next_obs[0][4] != 5 or next_obs[1][4] != 6:
        #     reward += 10
    ...
    def get_partial_obs(self, i):
        ...
        rel_dis_teammate = np.array(self.agt_pos[1-i]) - np.array(self.treasure_pos)
        rel_dis = np.array(self.agt_pos[i]) - np.array(self.treasure_pos)
        return np.concatenate([np.squeeze(partial_obs), rel_dis_teammate, rel_dis])
    ...
    def plot_scene(self, idx):
        ...
        plt.savefig('../env_plot/st_{}.jpg'.format(idx))
        plt.clf()
    def show_scene(self, idx):
        ...
        ax2.imshow(self.get_agt_obs(0))
        ax3.imshow(self.get_agt_obs(1))
        # plt.savefig('envs_{}.jpg'.format(idx))
        # plt.clf()
    ...
    def render(self, t):
        ...
        # rotate 90
        new_obs = cv2.rotate(new_obs, cv2.ROTATE_90_COUNTERCLOCKWISE)
        cv2.imshow('image', new_obs)
        
        # save 
        # cv2.imwrite('../env_plot/env_{}.jpg'.format(t), new_obs)
        cv2.waitKey(1)
